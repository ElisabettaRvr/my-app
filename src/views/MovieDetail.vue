...
<template>
    <div>
        <v-btn icon="mdi-arrow-left" variant="text" class="mb-4" @click="router.back()" />
        ...
        <div v-else-if="movie">
            ...
            <v-row class="mb-8">
                <v-col v-for="actor in cast" :key="actor.id" cols="6" sm="4" md="3" lg="2">
                    <v-card rounded="lg" elevation="1">
                        <v-img :src="actor.profile_path
                            ? `https://image.tmdb.org/t/p/w185${actor.profile_path}`
                            : 'https://via.placeholder.com/185x278?text=N/D'" height="160" cover />
            ...
            <v-card class="mb-6 pa-4" elevation="2" rounded="lg">
                ...
                <v-textarea v-model="newTesto" label="Scrivi la tua recensione..." variant="outlined" rows="3"
                    hide-details class="mb-3" />
                <v-btn color="red-darken-4" :loading="submitting" :disabled="!newTesto.trim()"
            ...
            <v-card v-for="rec in recensioni" :key="rec.id" class="mb-4" elevation="1" rounded="lg">
                <v-card-text>
